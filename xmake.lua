...
option("modules", { defines = "FROZEN_MODULE", default = false })
option("std_module", { defines = "FROZEN_STD_MODULE", default = false })
    ...
    for _, example in ipairs(os.files("examples/*.cpp")) do
        target("frozen.example." .. path.basename(example), function()
            ...
            if path.basename(example) == "html_entities_map" then
                add_cxxflags("-fconstexpr-steps=123456789", { tools = { "clangxx", "clang" } })
                add_cxxflags("-fconstexpr-ops-limit=12345678910", { tools = { "gxx", "gcc" } })
            end
        end)
    end
end
